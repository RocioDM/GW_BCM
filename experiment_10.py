...
print('random template, extracted')
...
X_train, X_test, y_train, y_test = train_test_split(Data, label, test_size=0.95, random_state=42, stratify=label)
...
print(f"train len: {len(train_distance_matrices)}")

# Initialize lists for test set
test_indices = []
test_measures = []
...
for i in range(X_test.shape[0]):
    ...
    # Compute pairwise Euclidean distance matrix
    dist_matrix_s = sp.spatial.distance.cdist(C_s, C_s)
    test_distance_matrices.append(dist_matrix_s)

print('test samples, extracted')
...
labels = lambda_matrix[:,0:1].squeeze()
matrix = lambda_matrix[:,1:]
