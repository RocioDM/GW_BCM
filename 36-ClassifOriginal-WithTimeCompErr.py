...
measures = []  # list of sample measures
matrices = []  # list of sample dissimilarity matrices
...
#
# # For unsupervised clustering / visualization we use FP λ by default
# all_features = all_features_fp
#
# ################################################################################
# # COMPUTE GW-BARYCENTRIC COORDINATES FOR TEMPLATES THEMSELVES ##################
# ################################################################################
# print("\nCompute GW-barycentric coordinates for templates (fixed-point only, for plotting)")
#
...
# ###################################################################################################
#
# print("\nVisualizing λ in the simplex Δ² with K-Means decision regions (FP λ)")
#
...
#
#     plt.title("K-Means decision regions in the simplex Δ² (fixed-point λ)", fontsize=18)
...
#
# ###################################################################################################
# ## VISUALIZATION OF GW-BARYCENTRIC COORDINATES (TSNE + MDS, FP λ) ################################
# ###################################################################################################
...
#     # ============================================================
#     # FIX: use a consistent coordinate for data + templates (and normalize)
#     L_data_1d = all_features / all_features.sum(axis=1, keepdims=True)
#     L_temp_1d = template_lambdas / template_lambdas.sum(axis=1, keepdims=True)
#
#     lambdas_1d = L_data_1d[:, 0]  # one coordinate determines the other
...
#         )
#
#     # Overlay template λ-coordinates (same coordinate as data)
#     template_lambdas_1d = L_temp_1d[:, 0]
...
#     # t-SNE embedding
#     # ---------------------------
#     n_samples_tsne = n_total
#     perplexity = min(30, max(5, n_samples_tsne // 3), n_samples_tsne - 1)
#
#     tsne = TSNE(
#         n_components=2,
...
#     for i in range(n_samples):
#         # FIX: use the DATA sample i (matrices/measures), not the template lists
#         dist_matrix_s = matrices[i]
#         p_s = measures[i]
#
...
#
#     run_times.append(time.perf_counter() - t0_run)
#     print(run_times[-1])
#
...
#
# print(f"GW 1-NN: mean acc = {all_gw_accuracies.mean():.4f} ± {all_gw_accuracies.std():.4f}")
# print(f"GW 1-NN: mean time/run = {run_times.mean():.4f} s ± {run_times.std():.4f} s")
# print(f"GW 1-NN: total time (all runs) = {run_times.sum():.4f} s")
#
#
# # 3) GW 1-NN baseline
# gw1nn_mean_acc = all_gw_accuracies.mean()
# gw1nn_std_acc = all_gw_accuracies.std()
# gw1nn_total_time = run_times.sum()  # FIX: total time across runs
#
# print(
#     f"GW 1-NN (templates): mean acc = {gw1nn_mean_acc:.4f}, std = {gw1nn_std_acc:.4f}, "
...
# -------------------------------------------------------------------
Cs_list = matrices  # list of distance matrices C_s
ps_list = measures  # list of probability vectors p_s

# -------------------------------------------------------------------
# 2) Learn GW dictionary on these Cs_list
# -------------------------------------------------------------------
D_dict = 2  # number of atoms in the dictionary
...
print(
    f"GW dictionary + k-NN: mean acc = {dict_mean_acc:.4f}, std = {dict_std_acc:.4f}, "
...
print(
    f"λ-space GW (fixed-point) + k-NN: mean acc = {lambda_fp_mean_acc:.4f}, "
...
print(
    f"λ-space GW (blow-up) + k-NN: mean acc = {lambda_bu_mean_acc:.4f}, "
...
"""
    % (
        lambda_fp_mean_acc,
        lambda_fp_std_acc,
        lambda_fp_total_time,
        lambda_bu_mean_acc,
        lambda_bu_std_acc,
        lambda_bu_total_time,
        gw1nn_mean_acc,
        gw1nn_std_acc,
        gw1nn_total_time,
        dict_mean_acc,
